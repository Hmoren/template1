...
export class ProductsService {
  ...
  private cargarProductos() { // Leer el archivo JSON
    this.http.get('assets/data/productos_idx.json').subscribe((resp: any) => {
      this.productos = resp;
    });
  }
  ...
  private cargarProductoDetalle() {
    ...
    return new Promise<void>((resolve, reject) => {
      this.http.get('assets/data/productos-detalle.json').subscribe((resp: any) => {
  ...
  getProducto(id: string, sku: string) {
    if (this.productoDetalle.length === 0) { // cargar productos
